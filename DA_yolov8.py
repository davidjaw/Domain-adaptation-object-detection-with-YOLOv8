...
def seed_everything(seed=66):
    # Can't get the same result even through set a common seed
    random.seed(seed)
...
class GradientReversalFunction(torch.autograd.Function):
    # Gradient Reversal, this is used to change the direction of gradient during gridient descent
    @staticmethod
...
class Discriminator(nn.Module):
    def __init__(self, chs=None, amp=False):
        ...
        super().__init__()
        if chs is None:
            # chs = [32, 64, 128] ##　for yolov8n
            # chs = [64, 128, 256] ## for yolov8s
            chs = [96,192,384] ## for yolov8m
            self.chs = chs
            self.f_len = len(chs)
...
class CustomTrainer(DetectionTrainer):
    ...
    def activate_hook(self, layer_indices = [2, 4, 6]):
    ...
    def optimizer_step(self, optims):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    if 10 < epoch < self.args.epochs - 50:
                        # hinge loss
                        threshold = 20
